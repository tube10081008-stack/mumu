...
import { Plus, Trash2, MapPin, Truck, Calendar, ArrowUp, ArrowDown, Edit2, X, Check } from 'lucide-react';

export default function AdminDashboard() {
    const [activeTab, setActiveTab] = useState('STATUS'); // STATUS, ROUTES, LOCATIONS
    ...
    const moveRoute = async (index, direction) => {
        ...
        // For MVP, simplistic approach: update both with new sequences
        // In production, use RPC or careful transaction
        const itemA = newRoutes[index];
        const itemB = newRoutes[targetIndex];

        await supabase.from('daily_routes').update({ sequence: index + 1 }).eq('id', itemA.id); // +1 because array is 0-indexed
    ...
    return (
        <div className="space-y-6">
            <div className="flex gap-2 p-1 bg-white rounded-2xl shadow-sm overflow-x-auto">
                {['STATUS', 'ROUTES', 'LOCATIONS'].map(tab => (
                    <button
                        key={tab}
                    ...
                    >
                        {tab === 'STATUS' ? '배송 현황' : tab === 'ROUTES' ? '경로 배정' : '배송지 관리'}
                    </button>
                ))}
            </div>
                            ...
                            <div className="bg-blue-50/50 border-2 border-blue-100 p-5 rounded-3xl h-[600px] flex flex-col">
                                ...
                                <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                    {driverRoutes.map((route, idx) => (
                                        <div key={route.id} className="flex items-center gap-3 p-3 bg-white border border-blue-100 rounded-xl shadow-sm animate-in slide-in-from-right-5 fade-in duration-300">
                                ...
                                <div className="flex gap-2 bg-slate-50 p-1 rounded-xl">
                                    {['NORTH', 'SOUTH'].map(r => (
                                        <button key={r} type="button"
                                            ...
                                            className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${locForm.region === r ? 'bg-white shadow text-toss-blue' : 'text-slate-400'}`}>
                                            {r === 'NORTH' ? '강북(북부)' : '강남(남부)'}
                                        </button>
                                    ))}
                                </div>
